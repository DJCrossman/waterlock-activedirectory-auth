...
exports.beforeCreate = function(values, cb){
  ...
  var config = require('../waterlock-activedirectory-auth').config;
  var issued = Date.now();
...
exports.afterCreate = function(token, cb){
  var config = require('../waterlock-activedirectory-auth').authConfig;
  ...
  if(config.passwordReset.tokens){
    ...
    Auth.findOne(token.owner).exec(function(err, u){
      ...
      var transport = require('../waterlock-activedirectory-auth').transport;
